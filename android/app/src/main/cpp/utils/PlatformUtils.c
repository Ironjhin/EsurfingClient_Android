...
bool load_cfg()
{
    ...
    if (!cfg_json)
    {
        LOG_FATAL("JSON 解析失败, 请通过设置页重新保存配置");
        g_prog_enabled = false;
        g_prog_cnt = 0;
        return true;
    }
    ...
    if (enabled == NULL)
    {
        LOG_WARN("enabled 参数不存在, 请通过设置页启用");
        g_prog_enabled = false;
        g_prog_cnt = 0;
    ...
    {
        LOG_WARN("配置文件中禁用了程序启动, 请通过设置页启用");
        g_prog_enabled = false;
        g_prog_cnt = 0;
        cJSON_Delete(cfg_json);
        return true;
